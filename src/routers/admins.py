...
def login(credenciais: LoginAdmin,db: Session = Depends(get_db)):
    ...
    if not user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Email ou Senha incorreta!")
    ...
    if verifica_senha(credenciais.senha,user.senha):
        ...
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,detail="Email ou Senha incorreta!")
...
def delete_admin(id: int, db: Session = Depends(get_db), payload: dict = Depends(decode_token)):
    ...
    if not deleted_user.ativo:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT,detail="Usuário já deletado!")
